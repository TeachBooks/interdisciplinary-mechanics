...
We now consider the two semi-infinite rods connected as shown in Figure {numref}`Figure {number} <A2_Figure_8>`. As indicated, a wave travels in rightward direction. Clearly, this incident wave will generate a reflected and a transmitted wave at the interface. Taking all together, we can write
