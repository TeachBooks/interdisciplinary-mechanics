...
The tidal wave motion can be treated as in part 1 of this assignment, as the real part of a complex quantity $\zeta(s,t)=\operatorname{Re} \left⁡\\{ \tilde{\zeta}(s)e^{i(\omega t)} \right\\}$. Substitute $\sqrt{\frac{gA_c}{B}}=c_0$​​ (being the phase (and wave) velocity in the absence of resistance) and apply $\zeta(s,t)=\operatorname{Re}\left\⁡{\tilde{\zeta}(s)e^{i(\omega t)}\right\}$ to the Telegraph equation to obtain the reduced Telegraph equation. Hint: the time dependence drops after deriving $\zeta$.
...
````{admonition} Model answer
:class: dropdown
The blue term in the previous answer is $\frac{gA_c}{B}=c_0^2$. Substitute $\zeta(s,t)=\operatorname{Re}
````