...
Figure1
```
Consider a progressive wave in a 7-m depth channel with an amplitude $\overline{\zeta}(s=0)=1.1\,\mathrm{m}$, an initial phase $\alpha=0\,\mathrm{rad}$ rad and a length $L=200\,\mathrm{km}$. In absence of damping/resistance the amplitude remains constant along the channel.
