...
This wave is a shock, whose speed is 
...
Now, we organize a multiclass traffic race. A platoon with $20 %$ of trucks is waiting upstream at $x = 0$ until the traffic signal turns green at $t = 0$, see {Add figure reference} that represents the initial Riemann problem. We label the car flow as $1$ with a density _____ and truck flow as $2$ with a density ______. {Add Figure reference} provides the fundamental diagrams in speed for both vehicle types and the model you should solve is given on slide 3 in the lecture 15.
